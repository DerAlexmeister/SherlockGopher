module github.com/ob-algdatii-20ss/SherlockGopher/analyser

go 1.14

require (
	github.com/golang-collections/collections v0.0.0-20130729185459-604e922904d3
	github.com/golang/protobuf v1.3.2
	github.com/jwalteri/GO v0.0.0-20200412151504-f840cd9b8d15
	github.com/micro/go-micro v1.18.0
	//github.com/micro/go-micro/v2 v2.4.0 TODO
	github.com/micro/go-micro/v2 v2.3.0
	github.com/neo4j-drivers/gobolt v1.7.4 // indirect
	github.com/neo4j/neo4j-go-driver v1.7.4
	google.golang.org/grpc v1.26.0
	github.com/ob-algdatii-20ss/SherlockGopher/sherlockcrawler/proto v1.0.0
)
